...
const drawCards = (prods) => {
    prods.forEach((prod) => {
        ...
        // imagen
        templateCard.querySelector("img").setAttribute("src", prod.image);
...
const drawCart = () => {
    ...
    Object.values(carrito).forEach((prod) => {
        ...
        // subtotal
        templateCartItem.querySelector("span").textContent = prod.quantity * prod.price;
...
const drawFooter = () => {
    ...
    if (Object.keys(carrito).length === 0) {
        ...
        const totalValue = Object.values(carrito).reduce((acc, { quantity, price }) => acc + quantity * price, 0);
